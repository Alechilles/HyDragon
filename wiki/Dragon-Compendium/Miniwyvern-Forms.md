---
title: "Miniwyvern Forms"
order: 7
published: true
draft: false
---

# Miniwyvern Forms

Parent: [Dragon Compendium](/mod/hydragon/dragon-compendium-index) | [Home](/mod/hydragon/)

The Soulbound Miniwyvern starts Wild. After the Soul Bond is active, you can choose an attunement and re-attune later when another combat style fits your adventure better. These are the seven available forms:

| Form | Player-facing identity |
|---|---|
| **Wild** | Strong neutral damage with no Essence Bond passive. |
| **Fire** | Burning attacks and frequent fireballs. |
| **Ice** | Slows enemies and can freeze targets that become fully chilled. |
| **Lightning** | Improves the owner's movement and disrupts enemies. |
| **Nature** | Provides sustained healing and brief roots. |
| **Toxic** | Reduces the enemy's outgoing damage. |
| **Void** | Makes enemies take more damage. |

Every form keeps the Miniwyvern's projectile-and-swoop foundation. The form changes the status effect or owner benefit, while [Leveling and Talents](/mod/hydragon/leveling-and-talents) lets you specialize its combat and durability branches.

## Choosing and changing a form

Use [Soul Bond and Attunement](/mod/hydragon/soul-bond-and-attunement) for the attunement steps and material costs. Start Wild when you want neutral damage, choose Fire or Ice for pressure and control, Lightning for faster movement and disruption, Nature for support, Toxic for damage reduction, or Void for amplifying damage taken.

**Earth and Wind Draconic Essences are crafting materials, not selectable Miniwyvern forms.** If a recipe gives you Earth or Wind Essence, use it for crafting; the form list above contains every available choice.

## Next steps

- Create or re-attune your companion through [Soul Bond and Attunement](/mod/hydragon/soul-bond-and-attunement).
- Spend points deliberately with [Leveling and Talents](/mod/hydragon/leveling-and-talents).
- Return to the [Soulbound Miniwyvern](/mod/hydragon/soulbound-miniwyvern) page for roster, growth, and revival details.
