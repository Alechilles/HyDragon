...
The wild `NordicDrake` enables `_CombatConfig: CAE_NordicDrake` and the
combat-action-evaluator graph in `Template_HyDragon_Dragon`. That graph combines
the Drake's melee action selection and ground positioning with its fireball,
flying flame-breath, ingress, pass, and recovery cycle. Its aerial phase is
entered and exited through `UseHealthPhaseFlight` health-range checks.
...
The grounded branch reproduces the wild Nordic Drake's CAE-driven ground
contract rather than Tamework's generic attack loop. It uses
`CAE_NordicDrake`, the same melee root interaction, desired attack distance,
combat behavior distance, movement weights, backward speed, turn behavior, and
recovery parameters as the wild role.
...
It uses the wild role's fireball and flying flame-breath root interactions and
the same attack distances, altitude bands, approach speeds, pass timing,
wander geometry, and recovery tuning. Every phase remains gated by a valid
`LockedTarget`, `AirborneMode=true`, and the `Fly` controller so a toggle or
target loss cancels stale aerial work immediately.
...
`Tamed_NordicDrake` supplies the Nordic-only CAE and aerial combat parameters
needed by the new component. Its current health, progression, interaction,
...
- `Tamed_NordicDrake` wires the Nordic combat component and all required wild
  CAE, grounded, and aerial attack parameters;
...
- grounded combat uses the Nordic CAE configuration and melee root;
...
priority, multiple grounded CAE attacks, the complete aerial fireball/breath
